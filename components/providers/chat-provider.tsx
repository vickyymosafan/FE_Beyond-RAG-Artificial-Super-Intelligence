...
import { getStoredSessionId, createSessionWithRetry } from "@/lib/api/session-service"
...
const simpleSessionApiAdapter: SessionApiAdapter = {
  getStoredSessionId,
  storeSessionId: (id: string) => {
    if (typeof window !== "undefined") {
      sessionStorage.setItem("smartchat-session-id", id)
    }
  },
  createSessionWithRetry: async (sessionId) => {
    const result = await createSessionWithRetry(sessionId)
    return result
  },
}
...
export function ChatProvider({
  ...
  sessionApiAdapter = simpleSessionApiAdapter,
}: ChatProviderProps) {
