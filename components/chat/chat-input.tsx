...
export function ChatInput() {
  ...
  // Refs to track input state for correct synchronization
  // We need to know the "base" input (before speaking) to append the transcript correctly
  // instead of appending to the *current* state which might already contain the transcript
  const inputRef = React.useRef(input);
  const baseInputRef = React.useRef("");

  // Keep inputRef synced with state
  React.useEffect(() => {
    inputRef.current = input;
  }, [input]);

  // Capture base input when listening starts
  React.useEffect(() => {
    if (isListening) {
  ...
  React.useEffect(() => {
    if (transcript && transcript.length > 0) {
      ...
      const separator =
        base.trim().length > 0 && transcript.trim().length > 0 ? " " : "";
